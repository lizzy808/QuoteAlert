//
//  UIColorSheet.m
//  Quote Alert
//
//  Created by Elizabeth Choy on 4/6/14.
//  Copyright (c) 2014 Elizabeth Choy. All rights reserved.
//

#import "UIColorSheet.h"

@implementation UIColorSheet

+ (UIColor *)stockDarkGreyColor
{
    return [UIColor colorWithRed:61/255.0 green:77/255.0 blue:82/255.0 alpha:1];
}

+ (UIColor *)stockLightGreyColor
{
    return [UIColor colorWithRed:151/255.0 green:193/255.0 blue:204/255.0 alpha:1];
}

+ (UIColor *)stockRedColor
{
    return [UIColor colorWithRed:255/255.0 green:87/255.0 blue:74/255.0 alpha:1];
}

+ (UIColor *)lightRedColor
{
    return [UIColor colorWithRed:247/255.0 green:100/255.0 blue:97/255.0 alpha:1];
}

+ (UIColor *)stockGreenColor
{
    return [UIColor colorWithRed:96/255.0 green:255/255.0 blue:120/255.0 alpha:1];
}

+ (UIColor *)stockOrangeColor
{
    return [UIColor colorWithRed:255/255.0 green:201/255.0 blue:87/255.0 alpha:1];
}

+ (UIColor *)stockYellowColor
{
    return [UIColor colorWithRed:224/255.0 green:255/255.0 blue:43/255.0 alpha:1];
}

+ (UIColor *)stockBlueColor
{
    return [UIColor colorWithRed:110/255.0 green:202/255.0 blue:255/255.0 alpha:1];
}

@end
